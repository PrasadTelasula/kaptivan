...
export const jobAPI = {
  ...
  async listJobs(context: string, namespace: string): Promise<JobSummary[]> {
    const response = await fetch(`${API_BASE_URL}/api/v1/topology/${context}/jobs?namespace=${namespace}`);
    if (!response.ok) {
  ...
  async getJobTopology(context: string, namespace: string, name: string): Promise<JobTopology> {
    const response = await fetch(`${API_BASE_URL}/api/v1/topology/${context}/job/${namespace}/${name}`);
    if (!response.ok) {
